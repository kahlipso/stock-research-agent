...
export default async function StockPage({ params }: { params: Promise<{ ticker: string }> }) {
  ...
  const fixture = MOCK_COMPANIES.find((item) => item.ticker === parsed.data)!;
  const revenueGrowth = latest?.revenue && prior?.revenue ? latest.revenue / prior.revenue - 1 : null;
  ...
  return <main className="mx-auto max-w-[1400px] space-y-8 px-4 py-8 sm:px-6">
    <MockBanner source={data.profile.source} retrievedAt={data.profile.retrievedAt} />
    ...
    <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
      <MetricSection title="Growth" rows={[{ label: "Revenue", value: formatCurrency(latest?.revenue ?? null, true) }, { label: "YoY revenue growth", value: formatPercent(revenueGrowth) }, { label: "Three-year revenue CAGR", value: formatPercent(cagr) }, { label: "Earnings growth", value: formatPercent(fixture.earningsGrowth) }, { label: "Free cash flow", value: formatCurrency(latest?.freeCashFlow ?? null, true) }, { label: "Free-cash-flow growth", value: formatPercent(fixture.fcfGrowth) }]} />
      <MetricSection title="Profitability" rows={[{ label: "Gross margin", value: formatPercent(latest?.grossMargin ?? null) }, { label: "Operating margin", value: formatPercent(latest?.operatingMargin ?? null) }, { label: "Net margin", value: formatPercent(latest?.netMargin ?? null) }, { label: "Free-cash-flow margin", value: formatPercent(fixture.fcfMargin) }, { label: "Return on invested capital", value: formatPercent(fixture.roic) }]} />
      <MetricSection title="Balance sheet" rows={[{ label: "Cash", value: formatCurrency(latest?.cash ?? null, true) }, { label: "Total debt", value: formatCurrency(latest?.debt ?? null, true) }, { label: netCash != null && netCash >= 0 ? "Net cash" : "Net debt", value: formatCurrency(netCash != null ? Math.abs(netCash) : null, true) }, { label: "Current ratio", value: fixture.currentRatio != null ? `${fixture.currentRatio.toFixed(2)}x` : "Missing" }, { label: "Debt / free cash flow", value: debtFcf != null ? `${debtFcf.toFixed(2)}x` : "Missing" }]} />
