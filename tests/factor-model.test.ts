...
describe("factor model", () => {
  it("uses V0 weights without confidence or risk in alpha", () => { const result = aggregateAlpha({ quality: 100, value: 0, growth: 0, momentum: 0 }); expect(result.alphaScore).toBe(25); });
  ...
  it("classifies entry and exit bands", () => { expect(candidateBand(.9)).toBe("CANDIDATE"); expect(candidateBand(.69)).toBe("EXIT_RANGE"); });
